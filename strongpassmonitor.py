...
def check_password_strength(password):
    ...
    feedback = [messages[key] for key, regex in criteria.items() if not re.search(regex, password)]
    if feedback:
        return f"{Fore.RED}Weak:\n- " + "\n- ".join(feedback) + f"{Style.RESET_ALL}"
...
def main():
    ...
    while True:
        try:
            ...
            # Check for password leaks only if the password is strong
